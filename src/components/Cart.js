...
export const Cart = ({ item, setItem }) => {
  // Increase quantity
  const increaseQuantity = (index) => {
    const updatedItems = [...item];
    updatedItems[index].quantity = updatedItems[index].quantity + 1 || 1; // Ensure quantity is a valid number
  ...
  const calculateTotalAmount = () => {
    return item.reduce((total, prod) => {
      const price = parseFloat(prod.price) || 0; // Ensure price is valid
      const quantity = parseInt(prod.quantity, 10) || 1; // Ensure quantity is valid
        ...
        {item.length === 0 ? (
          ...
          <section className="fill-cart">
            {item.map((prod, index) => (
              <div className="cart-product" key={index}>
                ...
                <div className="cart-product-details">
                  ...
                  <p>Subtotal: Rs. {(parseFloat(prod.price) || 0) * (prod.quantity || 1)}</p>
                  <button
            ...
            <div className="cart-amt">
              <h4>Total Amount: Rs. {calculateTotalAmount().toFixed(2)}</h4>
